...
def show_stuff(batch_x, batch_y, prd_flow, wrp_im):
    ...
    label_im = visualise_flow(batch_y[0,:,:,:])
...
def set_flow_shape_chairs(flow):
    flow.set_shape((384,512,2))
    return flow

def set_flow_shape_my_synthetic(flow):
    flow.set_shape((128,128,2))
    return flow
...
def flow_error(gt_flow, flow):
    [_,w,h,_] = flow.shape
    return tf.losses.absolute_difference(tf.image.resize_images(gt_flow,[w,h]), flow)
...
class UnSupFlowNet():
    ...
    def _dataset_pipeline(self, im1_filenames, im2_filenames, y, dataset_name, batch_size, im_width=128, im_height=128):
        ...
        if y is not None:
            ...
            if dataset_name=='flying_chairs':
                ...
                dataset_y = dataset_y.map(lambda flow: tf.image.resize_images(set_flow_shape_chairs(flow),(im_width, im_height)))
            else:
                dataset_y = dataset_y.map(lambda filename: tf.py_func(read_flow_npy, [filename], [tf.float32]))
                dataset_y = dataset_y.map(lambda flow: tf.image.resize_images(set_flow_shape_my_synthetic(flow),(im_width, im_height)))
    ...
    def train_network(self, im1_filenames, im2_filenames, im_width, im_height, y=None, dataset_name = 'my_synthetic', t=1, hm_epochs = 1,
    ...
    load_model_path = 'ignore', save_model_path = 'ignore', save_flow_im_path = 'ignore', save_step = 100, show_step=-1):
        ...
        #placeholders
        iterator = self._dataset_pipeline(im1_filenames, im2_filenames, y, dataset_name, batch_size, im_width=im_width, im_height=im_height)
        if y is not None:
        ...
        w_err1, p_im1 = warping_error(x[:,:,:,0], x[:,:,:,1], flow1, t)
        w_err2, p_im2 = warping_error(x[:,:,:,0], x[:,:,:,1], flow2, t)
        w_err3, p_im3 = warping_error(x[:,:,:,0], x[:,:,:,1], flow3, t)
        w_err4, p_im4 = warping_error(x[:,:,:,0], x[:,:,:,1], flow4, t)
        w_err5, p_im5 = warping_error(x[:,:,:,0], x[:,:,:,1], flow5, t)
        w_err6, p_im6 = warping_error(x[:,:,:,0], x[:,:,:,1], flow6, t)
        ...
        avg_abs_photometric_loss = tf.losses.absolute_difference(x[:,:,:,1], p_image[:,:,:,0])
        if y is not None:
    ...
    def run_network(self, im1_filenames, im2_filenames, im_width, im_height, load_model_path, batch_size = 16, save_flow_im_path = 'ignore'):
        """ runs a model """
        #placeholders
        iterator = self._dataset_pipeline(im1_filenames, im2_filenames, None, batch_size, im_width=im_width, im_height=im_height)
        im1, im2 = iterator.get_next()
        x = tf.concat((im1,im2), axis=3)
        global_step = tf.Variable(0, name='global_step', trainable=False)
        #estimate flow
